  ...
  return (
    ...
    >
      <Link href={`/products/${product.slug}`} className="block">
        <div className="relative aspect-[3/4] overflow-hidden bg-storm/5">
          <img
            src={product.hero}
            alt={product.name}
            loading={priority ? "eager" : "lazy"}
            className="absolute inset-0 w-full h-full object-cover transition-transform duration-[1400ms] ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:scale-[1.04]"
          />
          <img
            src={second}
            alt=""
            ...
            <span className="flex items-center gap-1.5 text-paper">
              {product.colorways.slice(0, 3).map((c) => (
                <span
                  key={c.name}
                  title={c.name}
                  className="w-2.5 h-2.5 rounded-full border border-paper/50"
        ...
        <div className="mt-4 flex items-baseline justify-between">
          <h3 className="font-display text-[20px] md:text-[22px] tracking-[-0.01em] leading-tight">
            {product.name}
          </h3>
          <div className="font-tag text-tag-sm whitespace-nowrap">${product.price}</div>
        </div>
        <div className="font-tag text-tag-xs text-ink/55 mt-1 capitalize">{product.category}</div>
      </Link>
    </motion.div>
  );
}
